...
import { Leaf, HeartPulse, TrendingUp, MessageSquare, Bell, Clock, CheckCircle2, AlertTriangle, XCircle } from "lucide-react";
...
function StatusIcon({ status }: { status: string }) {
  if (status === "green") return <CheckCircle2 className="text-green-500" size={18} />;
  if (status === "yellow") return <AlertTriangle className="text-yellow-500" size={18} />;
...
export default function Home({ language }: HomeProps) {
  ...
  const quickActions = [
    { path: "/disease", icon: Leaf, label: tx.detectDisease, color: "bg-green-500", emoji: "🌿" },
    { path: "/health", icon: HeartPulse, label: tx.healthCheckin, color: "bg-blue-500", emoji: "💊" },
    { path: "/mandi", icon: TrendingUp, label: tx.mandiPrices, color: "bg-orange-500", emoji: "📊" },
    { path: "/chat", icon: MessageSquare, label: tx.aiChat, color: "bg-purple-500", emoji: "🤖" },
  ];
            ...
            <div className={`rounded-lg p-3 status-${lastCheckin.status}`}>
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="font-semibold text-sm">
                  {lastCheckin.status === "green" ? tx.statusGreen : lastCheckin.status === "yellow" ? tx.statusYellow : tx.statusRed}
                </span>
              </div>
              <p className="text-xs opacity-80 line-clamp-2">{lastCheckin.aiAdvice}</p>
            </div>
            <Link href="/health">
              <Button variant="outline" className="w-full mt-2 farmer-btn" data-testid="button-health-checkin">
        ...
        <Card>
          ...
          <CardContent>
            <div className="flex items-center justify-between">
              <div>
                <p className="font-semibold text-sm">{lastDetection.diseaseName}</p>
                <p className="text-xs text-muted-foreground">
                  {new Date(lastDetection.createdAt).toLocaleDateString("en-IN")}
                </p>
              </div>
              <Badge
                variant="outline"
            ...
            <Link href="/disease">
              <Button variant="outline" className="w-full mt-2 farmer-btn" data-testid="button-detect-disease">
                {tx.detectDisease}
              </Button>
            </Link>
          </CardContent>
      ...
      {!lastCheckin && !lastDetection && (
        <Card className="border-dashed">
          <CardContent className="p-6 text-center space-y-3">
            ...
            <Link href="/health">
              <Button className="farmer-btn w-full" data-testid="button-start-health">
      ...
      <Link href="/alerts">
        <Card className="cursor-pointer border-orange-200 bg-orange-50 dark:bg-orange-950/20 dark:border-orange-800 hover:shadow-md transition-shadow">
          <CardContent className="p-3 flex items-center gap-3">
            ...
            <span className="text-sm font-medium text-orange-700 dark:text-orange-300">{tx.smsAlerts}</span>
